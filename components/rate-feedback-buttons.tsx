...
export function RateFeedbackButtons({
  ...
  const sendFeedback = async (type: "confirm" | "flag", message?: string) => {
    ...
    try {
      ...
      toast({
        title: type === "confirm" ? "Thanks for confirming" : "Report received",
        description:
          type === "confirm"
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <DialogFooter>
          ...
          <Button onClick={submitWrongRate} disabled={wrongRateSubmitting}>
            {wrongRateSubmitting ? "Sending…" : "Submit report"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  ...
  return (
    <div className={`flex flex-wrap items-center gap-2 ${className}`}>
      ...
      <Button
        ...
        disabled={loading || submitted === "confirm"}
      >
        {submitted === "confirm" ? <Check className="h-3.5 w-3.5 text-green-600" /> : <ThumbsUp className="h-3.5 w-3.5" />}
