...
export function MLPredictions({ currentRate, backtestAccuracy, explanation }: MLPredictionsProps) {
  ...
  useEffect(() => {
    // Simulate ML analysis (isAnalyzing already true from initial state)
  ...
  const getVolatilityBadge = (volatility: string) => {
    switch (volatility) {
      case "low":
        return <Badge variant="secondary" className="bg-green-500/10 text-green-500">Low Movement</Badge>
      case "medium":
        return <Badge variant="secondary" className="bg-yellow-500/10 text-yellow-500">Moderate Movement</Badge>
      case "high":
        return <Badge variant="secondary" className="bg-red-500/10 text-red-500">High Movement</Badge>
      default:
        return null
    }
  }
        ...
        <CardContent className="p-12">
          <div className="flex flex-col items-center justify-center gap-4">
            <div className="relative">
              ...
              <Sparkles className="h-6 w-6 absolute -top-1 -right-1 animate-bounce text-amber-600 dark:text-amber-400" />
            </div>
            <div className="text-center">
              <h3 className="font-semibold text-lg mb-2">Building your rate outlook...</h3>
              <p className="text-sm text-muted-foreground">Reviewing recent rate history and forecast models</p>
            </div>
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-muted-foreground">7-day ensemble outlook</span>
              </div>
              ...
              <div className="flex items-center gap-2 mt-2">
                {ensemblePrediction > currentRate ? (
                  <>
                    ...
                    <span className="text-sm text-muted-foreground">possible upward move</span>
                  </>
                ) : (
                  <>
                    ...
                    <span className="text-sm text-muted-foreground">possible downward move</span>
                  </>
                )}
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3 sm:gap-4">
              ...
              <div className="text-center p-3 sm:p-4 rounded-xl bg-muted/40">
                ...
                <div className="text-[10px] sm:text-xs text-muted-foreground">ML Models Used</div>
              </div>
            </div>
          </div>
        </CardContent>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {predictions.map((pred) => (
          <Card key={pred.timeframe} className="relative">
            <div className={`absolute top-0 left-0 right-0 h-1 ${ pred.direction === "up" ? "bg-red-500" : pred.direction === "down" ? "bg-green-500" : "bg-yellow-500" }`} />
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="p-2 rounded bg-muted/40 border border-border/40">
                  <div className="text-muted-foreground">Lower watch band</div>
                  <div className="font-medium text-green-500">{pred.support}</div>
                </div>
                <div className="p-2 rounded bg-muted/40 border border-border/40">
                  <div className="text-muted-foreground">Upper watch band</div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {models.map((model) => (
              <div key={model.name} className="flex items-center gap-3 sm:gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-2 mb-1">
                    <span className="font-medium text-sm truncate">{model.name}</span>
                    <div className="flex items-center gap-2 sm:gap-3 text-xs sm:text-sm shrink-0">
                      <span className="text-muted-foreground">
                        <strong className="text-foreground">{model.prediction.toFixed(2)}</strong>
                      </span>
              ...
              {ensemblePrediction > currentRate ? (
                <>
                  <li className="flex items-start gap-2">
                    ...
                    <span>The rate may <strong className="text-red-500">rise</strong>. If you need USD soon, plan earlier rather than waiting.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 mt-0.5 flex-shrink-0 text-green-600 dark:text-green-400" />
                    <span>If you are converting USD into LRD, waiting could get you slightly more LRD.</span>
                  </li>
                </>
              ) : (
                <>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="h-4 w-4 mt-0.5 flex-shrink-0 text-green-600 dark:text-green-400" />
                    <span>The rate may <strong className="text-green-500">ease lower</strong>. Waiting could help if you plan to buy USD.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <AlertTriangle className="h-4 w-4 mt-0.5 flex-shrink-0 text-amber-600 dark:text-amber-400" />
                    <span>If you are changing USD into LRD, consider acting sooner rather than later.</span>
                  </li>
                </>
              )}
              <li className="flex items-start gap-2">
                <Clock className="h-4 w-4 mt-0.5 flex-shrink-0 text-blue-600 dark:text-blue-400" />
