...
export function RateChangeAnimation({
  ...
  useEffect(() => {
    ...
    if (prev != null && rate !== prev && Number.isFinite(rate) && Number.isFinite(prev)) {
      queueMicrotask(() => setFlash(rate > prev ? "up" : "down"))
      prevRateRef.current = rate
      const t = setTimeout(() => {
        setFlash(null)
      }, 600)
  ...
  return (
    <span
      className={`inline-block transition-[background-color,box-shadow] duration-300 ease-out rounded-md ${className} ${ flash === "up" ? "animate-rate-flash-up" : flash === "down" ? "animate-rate-flash-down" : "" }`}
