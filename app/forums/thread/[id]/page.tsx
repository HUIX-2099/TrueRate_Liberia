...
const REPLY_PROMPTS = ["+1 — same experience", "Question: ", "Tip: ", "Be careful: "] as const
...
export default function ThreadPage() {
  ...
  const appendReplyPrompt = useCallback((snippet: string) => {
    setReplyBody((prev) => {
      const next = prev.trim() ? `${prev.trim()} ${snippet}` : snippet
      return next.length > REPLY_MAX ? next.slice(0, REPLY_MAX) : next
    })
  }, [])
  ...
  const addReply = useCallback(() => {
    ...
    const newReply: ForumReply = {
      ...
      author: "You",
      body: replyBody.trim().slice(0, REPLY_MAX),
      time: "Just now",
    }
    ...
    <div className="min-h-screen flex flex-col overflow-x-hidden">
      ...
      <main className="flex-1 py-6 sm:py-10 min-w-0 pb-20 md:pb-0 overflow-x-hidden">
        <div className="container mx-auto px-4 sm:px-6 max-w-3xl min-w-0">
          ...
          <Card className="rounded-2xl border-border/40 shadow-sm mb-4 sm:mb-6">
          ...
          <h2 className="text-lg font-semibold text-foreground mb-1 flex items-center gap-2">
            ...
            {t("forum.repliesHeading")} {totalReplies > 0 && `(${totalReplies})`}
          </h2>
          <p className="text-sm text-muted-foreground mb-4 leading-relaxed">{t("forum.replySectionBlurb")}</p>

          <div className="space-y-3 sm:space-y-4 mb-6 sm:mb-8">
            {displayReplies.length === 0 ? (
              <p className="text-sm text-muted-foreground py-4 rounded-xl border border-dashed border-border/60 bg-muted/20 px-4">
              ...
              displayReplies.map((r) => (
                <Card
                  key={r.id}
                  className="rounded-2xl border-border/40 transition-all duration-200 hover:border-primary/15 hover:shadow-sm"
                >
                  <CardContent className="p-3 sm:p-4">
                    <div className="flex gap-3">
                      <Avatar className="h-9 w-9 shrink-0 text-primary">
                        <AvatarFallback className="text-xs">
                          {r.author
                            .split(" ")
                            .map((n) => n[0])
          ...
          <Card className="rounded-2xl border-primary/25 shadow-md overflow-hidden">
            <CardHeader className="p-4 sm:p-6 space-y-2">
              <div className="flex items-center gap-2 text-primary">
                <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-muted/40 border border-border/40">
                  <Sparkles className="h-4 w-4 text-amber-600 dark:text-amber-400" aria-hidden />
                </span>
                <CardTitle className="text-base font-bold">{t("forum.addYourReply")}</CardTitle>
              </div>
              <p className="text-xs text-muted-foreground leading-relaxed">{t("forum.replyPlaceholder")}</p>
            </CardHeader>
            <CardContent className="space-y-4 p-4 sm:p-6 pt-0">
              <div className="flex flex-wrap gap-2">
                {REPLY_PROMPTS.map((p) => (
                  <button
                    key={p}
                    type="button"
                    onClick={() => appendReplyPrompt(p)}
                    className="rounded-full border border-primary/20 bg-primary/5 px-3 py-1 text-[11px] font-medium text-primary transition-all hover:bg-muted/40 border-border/40 active:scale-95"
                  >
                    {p}
                  </button>
                ))}
              ...
              <Textarea
                ...
                rows={5}
                value={replyBody}
                onChange={(e) => setReplyBody(e.target.value.slice(0, REPLY_MAX))}
                className={cn(
                  "min-h-[120px] resize-y rounded-xl border-border/70 transition-shadow",
                  "focus-visible:ring-2 focus-visible:ring-primary/30"
                )}
              />
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <span className="text-[11px] tabular-nums text-muted-foreground">
                  {replyBody.length}/{REPLY_MAX}
                </span>
                <Button
                  onClick={addReply}
                  disabled={!replyBody.trim()}
                  className="gap-2 rounded-xl min-h-[44px] w-full sm:w-auto touch-manipulation shadow-sm transition-transform"
                >
