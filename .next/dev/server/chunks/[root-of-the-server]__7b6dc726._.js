...
"[project]/app/api/liberia-cpi/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
const extractCpiMetrics = (rows)=>{
    let cpi = null;
    let inflationMoM = null;
    let inflationYoY = null;
    let referenceMonth = "";
    rows.forEach((row)=>{
...
const resolveFetchUrl = (url, useProxy)=>useProxy ? `${ALLORIGINS}${encodeURIComponent(url)}` : url;
const parseTradingEconomics = (html)=>{
...
const fetchTradingEconomics = async ()=>{
    ...
    const [cpiRes, inflRes] = await Promise.all([
        fetchWithTimeout(cpiUrl, {
            next: {
                revalidate: 3600
            }
        }, FETCH_TIMEOUT_MS),
        fetchWithTimeout(inflUrl, {
            next: {
                revalidate: 3600
            }
        }, FETCH_TIMEOUT_MS)
    ...
    const [cpiHtml, inflHtml] = await Promise.all([
        cpiRes.text(),
        inflRes.text()
    ]);
...
const fetchLisgisExcel = async ()=>{
    ...
    const candidates = rows.map((row)=>{
        ...
        const link = $(row).find('a[href*="Liberia_CPI_"][href$=".xlsx"], a[href*="Liberia_CPI_"][href$=".xls"]').attr("href");
    ...
    const excelRes = await fetchWithTimeout(excelFetchUrl, {
        next: {
            revalidate: EXCEL_REVALIDATE_SECONDS
        }
    }, FETCH_TIMEOUT_MS);
    ...
    const rowsData = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$xlsx$2f$xlsx$2e$mjs__$5b$app$2d$route$5d$__$28$ecmascript$29$__["utils"].sheet_to_json(sheet, {
        header: 1,
        defval: ""
    });
    ...
    return {
        ...
        lastMonth: metrics.referenceMonth || latest.date.toLocaleDateString("en-US", {
            month: "long",
            year: "numeric"
...
async function GET() {
    try {
        ...
        if (trading?.cpi) {
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(trading);
        }
    } catch (error) {
    ...
    try {
        ...
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(lisgis);
    } catch (error) {
        console.error("[Liberia CPI] LISGIS fallback failed", error);
        return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json(LISGIS_FALLBACK, {
            status: 200
        });
    }
}
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7b6dc726._.js.map