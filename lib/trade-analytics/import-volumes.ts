...
export function analyzeImportVolumes(records: ImportRecord[]): ImportVolumeAnalysis[] {
  ...
  for (const [period, list] of byPeriodMap) {
    ...
    result.push({
      ...
      totalValueUsd: (totalValueUsd ?? 0) > 0 ? Number((totalValueUsd ?? 0).toFixed(2)) : undefined,
      totalValueLocal: (totalValueLocal ?? 0) > 0 ? Number((totalValueLocal ?? 0).toFixed(2)) : undefined,
      byCategory,
