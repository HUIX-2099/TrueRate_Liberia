 ...
 */
export function CountyFlag({ county, className = "h-6 w-8 rounded" }: { county: string; className?: string }) {
  ...
  return (
    <img
      ...
      className={`inline-block object-cover rounded border border-border/50 shadow-sm ${className}`}
      title={county}
      loading="lazy"
    />
  )
