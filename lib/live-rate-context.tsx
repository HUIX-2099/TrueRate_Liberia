...
import { CANONICAL_FALLBACK_RATE, getCanonicalFallbackRate } from "@/lib/canonical-rate"
import { clearFetchCache, clearServiceWorkerCaches } from "@/lib/utils"
...
function readCachedRate(): number {
  if (typeof window === "undefined") return getCanonicalFallbackRate()
  try {
    const s = window.localStorage.getItem(RATE_STORAGE_KEY)
    if (!s) return getCanonicalFallbackRate()
    const n = Number.parseFloat(s)
    return Number.isFinite(n) && n > 100 && n < 300 ? n : getCanonicalFallbackRate()
  } catch {
...
export function LiveRateProvider({ children }: { children: ReactNode }) {
  // Use canonical fallback for initial state so server and client render the same (avoids hydration mismatch).
  // Cache is applied in useEffect after mount.
  const [rate, setRate] = useState<number>(CANONICAL_FALLBACK_RATE)
...
/** Stable fallback when used outside LiveRateProvider; avoids window/localStorage during render for SSR consistency. */
const FALLBACK_VALUE: LiveRateContextValue = {
  rate: CANONICAL_FALLBACK_RATE,
  loading: true,
  refresh: async () => {},
  ...
  rateSource: "market",
  setRateSource: () => {},
  effectiveRate: CANONICAL_FALLBACK_RATE,
}
